...
static void show_trace(struct task_struct *task, struct pt_regs *regs,
			unsigned long *stack, int depth, const char *log_lvl)
{
	struct unwind_state state;
	struct stack_info stack_info = {0};
	unsigned long visit_mask = 0;
...
void trace_call(struct task_struct *task, unsigned long *sp, int depth,
		       const char *loglvl)
{
	show_trace(current, NULL, NULL, depth, KERN_DEFAULT);
}
